    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            ...
            if "'NoneType' object is not iterable" not in err_text:
                raise
            logger.debug(
                "Codex Responses SDK stream iterator was None; falling back to "
    ...
    try:
        for event in stream_or_response:
            ...
            if terminal_response is not None:
                ...
                # already carried the real message items. Leaving output as
                # None makes the SDK's output_text property raise
                # TypeError("'NoneType' object is not iterable") later in the
                # conversation loop.
